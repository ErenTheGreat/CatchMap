#!/usr/bin/env python3
"""Execute batches 001-034 via Supabase MCP execute_sql (stdio transport).

Requires SUPABASE_ACCESS_TOKEN in environment (from supabase.com/dashboard/account/tokens).
Uses @supabase/mcp-server-supabase over stdio with MCP Python SDK.
"""
    ...
    async with stdio_client(server_params) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            for q in [
                ...
                "SELECT count(*)::int AS total FROM public.locations;",
            ]:
                r = await session.call_tool("execute_sql", {"project_id": PROJECT_ID, "query": q})
                print(f"POST {q[:40]}... -> {r.content}")

...
export function useViewportSpotScores({
  ...
  const scoresBySpotId = useMemo(() => {
    ...
    const scores = scoreSpotsForDiscovery(
      spots,
      { weather, tides, personalBoost, trustBoostBySpotId },
      enrichment,
      communityBySpotId
    );
  ...
  }, [enabled, spotKey, hourBucket, weatherBucket, tides, spots, communityBySpotId, personalBoost, trustBoostBySpotId, weather]);

  const rankedDiscoverySpots = useMemo(() => {
    if (!enabled || spots.length === 0) return [];
    return rankDiscoverySpots(spots, scoresBySpotId);
  }, [enabled, spots, scoresBySpotId, spotKey, hourBucket, enrichedScoresVersion]);

  const topSpots = useMemo(() => {
    if (enrichedTopSpots.length > 0) return enrichedTopSpots;
    return rankedDiscoverySpots.slice(0, TOP_SPOTS_DISPLAY);
  }, [enrichedTopSpots, rankedDiscoverySpots]);

  const speciesBySpotId = useMemo(() => {
    const record: Record<string, SpeciesPrediction[]> = {};
    enrichedBySpotIdRef.current.forEach((predictions, spotId) => {
      record[spotId] = predictions;
    });
    return record;
  }, [enrichedScoresVersion]);

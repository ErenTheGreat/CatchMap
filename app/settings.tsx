...
export default function SettingsScreen() {
  ...
  const { isPro, priceLabel, monthlyPriceLabel, purchasePro, purchaseProMonthly, restorePurchases } =
    usePro();
  const { reset: resetOnboarding } = useOnboarding();
  const { reset: resetSubscriptionGate } = useSubscriptionGate();
  const [purchasingPro, setPurchasingPro] = useState(false);
  const [purchasingProMonthly, setPurchasingProMonthly] = useState(false);
  ...
  const handlePurchaseProMonthly = async () => {
    setPurchasingProMonthly(true);
    const { error, entitled } = await purchaseProMonthly();
    setPurchasingProMonthly(false);
    if (error) {
      showToast({ message: error, variant: 'error' });
      return;
    }
  ...
  const handleResetFirstLaunch = () => {
    Alert.alert(
      'Reset first launch',
      'This clears onboarding and the Pro welcome screen so you can test the first-launch flow again. Restart the app to see it.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            hapticSuccess();
            showToast({
              message: 'First-launch state cleared — restart the app',
              variant: 'success',
            });
          },
        },
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.aiDescription}>
            {isPro
              ...
              : `${monthlyPriceLabel} monthly or ${priceLabel} lifetime. Hosted Catch AI (30 requests/day), cloud sync, offline maps, trip planner, and more.`}
          </Text>
          {PRO_LAUNCH_PROMO_ACTIVE && !isPro ? (
            <Text style={styles.promoNote}>Lifetime launch promo available — monthly recommended</Text>
          ) : null}
          {!isPro ? (
            <>
              <Button
                title={purchasingProMonthly ? 'Processing…' : `Subscribe — ${monthlyPriceLabel}`}
                onPress={handlePurchaseProMonthly}
                loading={purchasingProMonthly}
                style={styles.aiButton}
              />
              <Button
                title={purchasingPro ? 'Processing…' : `Lifetime — ${priceLabel}`}
          ...
          <>
            ...
            <View style={styles.card}>
              <TouchableOpacity
                ...
                accessibilityRole="button"
                accessibilityLabel="Reset first launch flow"
              >
                <Info color={colors.accent} size={18} />
                <View style={styles.actionTextBlock}>
                  <Text style={styles.actionTitle}>Reset first launch</Text>
                  <Text style={styles.actionSubtitle}>
                    Show onboarding and Pro welcome again after restart
                  </Text>
                </View>
              </TouchableOpacity>
            </View>

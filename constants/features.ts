...
/** Bundled "what to catch + where to fish" discovery card (Pro). */
export function isFishTodayBundleEnabled(): boolean {
  return isProFeatureEnabled('ai_fish_today');
}

export function isAiFishTodayEnabled(): boolean {
  return isFishTodayBundleEnabled();
}

/** Viewport species targets from the scoring engine (not generative AI). */
export function isTodaySpeciesTargetsEnabled(): boolean {
  return isFishTodayBundleEnabled();
}

/** Side-by-side compare for close-ranked spots (bundled with trip planner). */
export function isProSpotCompareEnabled(): boolean {
  return isProFeatureEnabled('trip_planner');
}

/** Weekend Sat/Sun outlook across saved spots (bundled with trip planner). */
export function isWeekendPlannerEnabled(): boolean {
  return isProFeatureEnabled('trip_planner');
}

/** Bite Storm — push when conditions flip hot at saved spots (bundled with pattern alerts). */
export function isBiteStormEnabled(): boolean {
  return isProFeatureEnabled('pattern_alerts');
}

/** Autopilot Saturday — full-day itinerary across saved spots (bundled with trip planner). */
export function isAutopilotSaturdayEnabled(): boolean {
  return isProFeatureEnabled('trip_planner');
}

/** On-Water Copilot — GPS-aware fishing advisor (bundled with hosted AI). */
export function isOnWaterCopilotEnabled(): boolean {
  return isCatchAiChatEnabled();
}

/** Lure Pulse — trending community lures in viewport (bundled with premium map layers). */
export function isLurePulseEnabled(): boolean {
  return isPremiumMapLayersEnabled();
}

/** Spot Trust Score — personal prediction accuracy per saved spot (bundled with personal insights). */
export function isSpotTrustScoreEnabled(): boolean {
  return hasProPersonalInsights();
}

/** Water Whisper — AI tactical read from a water photo (bundled with hosted AI / species ID). */
export function isWaterWhisperEnabled(): boolean {
  return isSpeciesIdEnabled() || isCatchAiChatEnabled();
}

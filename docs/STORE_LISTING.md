...
**Learn as you go** — Species guide with rig diagrams, regulations notices, and Catch Coach tips. **CatchMap Pro** adds hosted Catch AI, photo species ID, trip planner, and cloud backup.
...
**CatchMap Pro** — Monthly subscription ($4/mo) or one-time lifetime purchase ($49.99 launch / $59.99 list). Unlock hosted AI, cloud sync, trip planner, pattern alerts, and personal insights.

Free map and catch logging. Pro unlocks the serious angler toolkit.

> **v1.0 note:** Offline map downloads are not included in the initial public release (production builds use the WebView map). Do not advertise offline maps in store copy until native MapLibre ships in release builds.
...
6. First launch shows onboarding, then the monthly Pro gate; users can continue with the free plan
...
6. **Catch AI** — Chat assistant (Pro, hosted — no API key required)
7. **Trip planner** — Ranked spots for today/tomorrow
8. **Settings** — Account, Pro upgrade, export, privacy/terms links

Recommended sizes: iPhone 6.7" and 6.1", iPad 12.9" (if supporting tablet prominently), Android phone 1080×1920.

## Pre-submit checklist

- [ ] Production EAS build smoke-tested (map, log, sign-up, sync, export, Pro IAP sandbox, Catch AI)
...
- [ ] RevenueCat keys in EAS production (`npm run setup:revenuecat` or `eas env:list`)
- [ ] Supabase Edge Function secrets: `GEMINI_API_KEY`, `REVENUECAT_SECRET_API_KEY`
- [ ] RevenueCat webhook → `https://cpzwvlpqdzjjsdlnmfgg.supabase.co/functions/v1/revenuecat-webhook`
- [ ] `npm run verify:pro-launch` passes

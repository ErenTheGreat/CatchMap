#!/usr/bin/env node
/**
 * Import Northeast batch SQL via Supabase Management API (same backend as MCP execute_sql).
 * Usage: SUPABASE_ACCESS_TOKEN=... node scripts/northeast-import-mcp-api.js
 */
const fs = require('fs');
...
const BATCH_DIR = path.join(__dirname, '../.import/mcp_run/northeast');
const PARALLEL = 3;
const RESULT_FILE = path.join(BATCH_DIR, '_northeast_import_result.json');
...
async function countNortheast(label) {
  ...
  console.log(`${label}: total=${row.total}, northeast=${row.northeast}`);
  return row;
}
...
async function main() {
  ...
  const before = await countNortheast('Before');
  const results = [];
  ...
  const after = await countNortheast('After');
  ...
  const summary = {
    ...
    northeast_before: before.northeast,
    northeast_after: after.northeast,
    northeast_delta: after.northeast - before.northeast,
    before,
    after,
    failed,
  };

  ...
  return (
    <View style={[styles.chip, isSelected && styles.chipSelected]}>
      ...
      >
        ...
        {trustLabel ? <Text style={styles.trustLabel}>{trustLabel}</Text> : null}
  ...
  const renderSaved = useCallback(
    ({ item }: { item: SavedSpotSnapshot }) => (
      <SpotChip
        ...
        trustLabel={
          (trustBySpotId[item.id]?.sampleSize ?? 0) >= 2 ? trustBySpotId[item.id]?.label : null
        }
      />
    ),
    [colors, handleNavigate, onSpotPress, selectedSpotId, styles, trustBySpotId]
  );

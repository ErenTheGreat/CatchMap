...
const AUTOPILOT_REMINDERS_KEY = '@autopilot_reminders_v1';
...
export interface StoredAutopilotReminder {
  notificationId: string;
  windowStartIso: string;
  windowEndIso: string;
  spotName: string;
...
export async function scheduleTripReminder(
  ...
  const speciesSpotLabel =
    speciesName && spotName ? `${speciesName} — ${spotName}` : speciesName ?? spotName;

  const title =
    patternMatchScore != null && patternMatchScore >= 70 && speciesSpotLabel
      ? `Pattern match (${patternMatchScore}%) — ${speciesSpotLabel}`
      : speciesSpotLabel
        ? `Fishing soon — ${speciesSpotLabel}`
        : 'Fishing bite window soon';

  let body = `${formatTripWindowSummary(tripWindow, spotName)}. Window: ${formatTripWindowRange(tripWindow)}.`;
  if (speciesName) {
    body = `Target ${speciesName}. ${body}`;
  }
...
export async function loadAutopilotReminders(): Promise<StoredAutopilotReminder[]> {
  try {
    const raw = await AsyncStorage.getItem(AUTOPILOT_REMINDERS_KEY);
    if (!raw) return [];
    return JSON.parse(raw) as StoredAutopilotReminder[];
  } catch (error) {
    if (__DEV__) console.warn('[tripReminders] corrupt autopilot reminders:', error);
    return [];
  }
}

async function persistAutopilotReminders(reminders: StoredAutopilotReminder[]): Promise<void> {
  if (reminders.length === 0) {
    await AsyncStorage.removeItem(AUTOPILOT_REMINDERS_KEY);
    return;
  }
  await AsyncStorage.setItem(AUTOPILOT_REMINDERS_KEY, JSON.stringify(reminders));
}

export async function cancelAutopilotReminders(): Promise<void> {
  const stored = await loadAutopilotReminders();
  ...
  if (isNotificationsAvailable()) {
    try {
      ...
      await Promise.all(
        stored.map((reminder) =>
          Notifications.cancelScheduledNotificationAsync(reminder.notificationId).catch(() => undefined)
        )
      );
    } catch (error) {
      if (__DEV__) console.warn('[tripReminders] cancel autopilot reminders failed:', error);
    }
  }

  await persistAutopilotReminders([]);
}

export interface AutopilotReminderLeg {
  legIndex: number;
  tripWindow: TripWindow;
  spotName: string;
...
export type ScheduleAutopilotRemindersResult =
  | { ok: true; scheduled: Array<{ legIndex: number; fireAt: Date; notificationId: string }> }
  | { ok: false; reason: 'permission_denied' | 'unavailable' | 'web' | 'none_scheduled' };

export async function scheduleAutopilotReminders(
  legs: AutopilotReminderLeg[],
  leadMinutes = DEFAULT_REMINDER_LEAD_MINUTES
): Promise<ScheduleAutopilotRemindersResult> {
  if (!isNotificationsAvailable()) {
    return { ok: false, reason: 'web' };
  }
  ...
  const Notifications = await getNotificationsModule();
  const scheduled: Array<{ legIndex: number; fireAt: Date; notificationId: string }> = [];
  const stored: StoredAutopilotReminder[] = [];

  for (const leg of legs) {
    const fireAt = computeReminderFireTime(leg.tripWindow.startTime, new Date(), leadMinutes);
    if (!fireAt) continue;

    const speciesSpotLabel =
      leg.speciesName && leg.spotName
        ? `${leg.speciesName} — ${leg.spotName}`
        : leg.spotName;

    const title = `Autopilot Saturday — Stop ${leg.legIndex}: ${speciesSpotLabel}`;
    const body = `Leg ${leg.legIndex} starts soon. ${formatTripWindowSummary(leg.tripWindow, leg.spotName)}`;
    ...
    try {
      ...
      scheduled.push({ legIndex: leg.legIndex, fireAt, notificationId });
      stored.push({
        notificationId,
        windowStartIso: leg.tripWindow.startTime.toISOString(),
        windowEndIso: leg.tripWindow.endTime.toISOString(),
        spotName: leg.spotName,
        speciesName: leg.speciesName ?? undefined,
        legIndex: leg.legIndex,
        latitude: leg.latitude,
        longitude: leg.longitude,
      });
    } catch (error) {
      if (__DEV__) console.warn('[tripReminders] autopilot leg schedule failed:', error);
    }
  }

  if (scheduled.length === 0) {
    return { ok: false, reason: 'none_scheduled' };
  }

  await persistAutopilotReminders(stored);
